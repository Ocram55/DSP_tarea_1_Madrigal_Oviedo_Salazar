...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [audioarray, fs, delta0, delta1, retardo0, retardo1, win_len]=deco_init()
  printf("\n----------------------------------------------------------------\n");
  ...
  if(length(retardo0) == 0) retardo0 =round(150e-6*fs) ; endif
  if(length(retardo1) == 0) retardo1 = round(300e-6*fs); endif
  printf("*******************************************************************\n");
  printf("El valor recomendado minimo de tiempo por ventana es %f us.\n", (max(retardo0,retardo1)/fs)*8*1e6);
  printf("*******************************************************************\n");
  ...
  if(length(win_len) == 0) win_len = round(1500*1e-6*fs) ; endif
endfunction